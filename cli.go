...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install SuperCord")
	var updateFlag = flag.Bool("repair", false, "Repair SupêrCord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall SuperCord")
	...
	if !slices.ContainsFunc(switches, func(b *bool) bool { return *b }) {
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update SuperCord Installer' option to update, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update SuperCord Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest SuperCord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, SuperCord will likely not work.")
}
